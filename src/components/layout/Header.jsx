...
export function Header() {
  ...
  return (
    <header className={`sticky top-0 z-30 flex h-16 shrink-0 w-full items-center justify-between border-b px-4 sm:px-6 backdrop-blur-md transition-colors duration-500 ${role === 'Admin' ? 'bg-white/80 dark:bg-[var(--color-fin-bg)]/80 border-slate-200 dark:border-slate-800/50' : 'bg-emerald-50/90 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/40'}`}>
      <div className="flex items-center">
        <div className={`flex h-7 w-7 items-center justify-center rounded-lg text-white shadow-sm md:hidden mr-3 shrink-0 transition-colors ${role === 'Admin' ? 'bg-[var(--color-fin-primary)]' : 'bg-emerald-500'}`}>
